#!/usr/bin/env bash
# panda mock 풀 앱 스택 컨테이너 실행 (X11 forwarding).
#
#   ros2 launch rdfp rdfp_panda_mock.launch.py 를 컨테이너에서 띄운다.
...
#                     예: RDFP_CONFIG_DIR=$PWD/src/rdfp/config ./docker/run_panda_mock.sh
#
# 웹캠이 없거나 카메라를 끄고 싶으면 launch 인자를 그대로 넘긴다:
#   ./docker/run_panda_mock.sh enable_camera_node:=false
set -euo pipefail

IMAGE="${IMAGE:-rdfp-panda-mock:latest}"
...
if [[ $# -gt 0 && "$1" != ros2 && "$1" != /* && "$1" != bash ]]; then
  set -- ros2 launch rdfp rdfp_panda_mock.launch.py "$@" "${AUTO_CONFIG_ARGS[@]}"
elif [[ $# -eq 0 ]]; then
  set -- ros2 launch rdfp rdfp_panda_mock.launch.py "${AUTO_CONFIG_ARGS[@]}"
fi

exec docker run --rm -it \
  --name rdfp-panda-mock \
  --network host \
  "${GPU_ARGS[@]}" \
  "${CAM_ARGS[@]}" \
  "${CONFIG_ARGS[@]}" \
