...
"""Isaac 백엔드 **Phase 4 수용 기준**(고정 시점 카메라)을 검사한다.
    ...
    3. 스탬프       — `header.stamp` 이 채워지고 **sim time 으로 진행**하는가
    4. camera_info  — 함께 오고 해상도가 이미지와 일치하는가
...
def check_stamp(node: Phase4Checker) -> bool:
    ...
    print(f'  3. 스탬프             : {_fmt(ok)} — 첫 {stamps[0]:.2f} 끝 {stamps[-1]:.2f} '
          f'(경과 {span:.2f}s)')
    if not filled:
...
def check_camera_info(node: Phase4Checker, cam: dict) -> bool:
    if not node.infos:
        print(f'  4. camera_info        : {_fmt(False)} — 수신 없음')
        return False
    info = node.infos[-1]
    width, height = cam['resolution']
    ok = info.width == width and info.height == height
    fx = info.k[0] if len(info.k) > 0 else 0.0
    print(f'  4. camera_info        : {_fmt(ok)} — {info.width}x{info.height}, '
