...
### 4.3 명령을 멈추면 팔이 처진다 — 발행은 끊지 않는다

**측정 중 스크립트 사이의 무명령 구간에서 EE 가 40 mm 내려앉았다.** 마지막 명령이
유지되지 않고 중력 평형으로 떨어진다. 보간이 없다는 성질(§4)과 별개의 문제다.

증상이 **"명령 무반응 고착"(A-4)과 똑같이 보인다** — 목표를 주고 잠시 쉬었다가 측정하는
루프를 돌리면, 매 반복 같은 평형으로 되돌아가므로 위치가 고정된 것처럼 관측된다.
실제로 그렇게 오진했다가 `joint1`(중력 무관 축)에 단독 명령을 넣어 정상 응답
(+0.0526 / 지령 +0.05)을 확인하고서야 구분했다.

> **구현 규칙.** 자세를 유지하는 동안에도 **50 Hz 발행을 계속한다.** "목표를 한 번
> 보내고 끝"인 구현은 조용히 무너진다. 파지·정지·대기 구간도 예외가 아니다.
>
> **진단 규칙.** 무반응처럼 보이면 A-4 로 단정하기 전에 **`joint1` 또는 `joint7`**
> (중력 토크가 걸리지 않는 축)에 단독 명령을 넣어 본다. 그 축이 응답하면 고착이
> 아니라 중력·부하 문제다.
