...
### 3.6 Docker volume 및 device mapping 구조

ROBOTIS Docker 컨테이너는 OMY-L100 하드웨어 접근, ROS 2 workspace 보존, RViz/Gazebo GUI 실행을 위해 host resource를 컨테이너 내부로 mapping합니다. 일반적으로 사용자는 `docker run` 명령을 직접 작성하기보다는, ROBOTIS가 제공하는 `./docker/container.sh` 스크립트를 통해 컨테이너를 시작합니다.

핵심 mapping 구조는 다음과 같이 이해하면 됩니다.

| Host path / resource | Container path / resource | 목적 |
|---|---|---|
| `~/robotis_omy/open_manipulator/workspace` 또는 ROBOTIS 스크립트가 지정한 workspace directory | `/workspace` | ROS workspace, build 결과, 사용자가 생성한 작업 파일 보존 |
| `/dev` | `/dev` | U2D2, `/dev/ttyUSB0` 등 USB serial hardware device 접근 |
| `/tmp/.X11-unix` | `/tmp/.X11-unix` | RViz, Gazebo 등 X11 GUI application 실행 |
| `$DISPLAY` | `$DISPLAY` | Host X server로 GUI window forwarding |
| Host network | Host network | ROS 2 DDS discovery와 topic/service 통신 단순화 |
| Docker container 내부 user/root filesystem | Container filesystem | 패키지 설치, 임시 파일, 설정 파일 저장. 단, container 삭제 시 사라질 수 있음 |

중요한 점은 **보존해야 하는 파일은 `/workspace` 아래에 저장하는 것**입니다. 컨테이너 내부에서 `/root`, `/home`, `/tmp` 등에 저장한 파일은 컨테이너를 삭제하거나 재생성하면 사라질 수 있습니다. 반면 `/workspace`는 host directory와 volume mapping되어 있으므로, 컨테이너를 재시작해도 유지됩니다.

예를 들어 컨테이너 안에서 직접 만든 launch file, calibration file, rosbag, dataset, log file 등을 보존하고 싶다면 다음과 같이 `/workspace` 아래에 저장하는 것이 안전합니다.

```bash
mkdir -p /workspace/my_omy_l100_tests
cd /workspace/my_omy_l100_tests
```

OMY-L100의 U2D2가 host에서 `/dev/ttyUSB0`로 보인다면, 컨테이너 내부에서도 다음 명령으로 확인할 수 있어야 합니다.

```bash
ls -l /dev/ttyUSB*
```

컨테이너 내부에서 `/dev/ttyUSB0`가 보이지 않는다면 다음 순서로 확인합니다.
...
RViz 또는 Gazebo GUI 실행에는 X11 socket과 display 환경 변수가 필요합니다. Host에서 다음 명령을 먼저 실행합니다.

```bash
xhost +local:docker
```

컨테이너 내부에서 display 환경 변수를 확인합니다.

```bash
echo $DISPLAY
```

ROBOTIS 스크립트가 실제로 어떤 volume과 device mapping option을 사용하는지 확인하고 싶다면 host에서 다음 명령으로 `container.sh` 내용을 확인할 수 있습니다.

```bash
cd ~/robotis_omy/open_manipulator
grep -n "workspace\|/dev\|X11\|DISPLAY\|network\|-v\|--volume" docker/container.sh
```

실전에서는 다음 원칙을 따르면 됩니다.

1. OMY-L100을 연결한 뒤 Docker container를 시작합니다.
2. 컨테이너 안에서 `/dev/ttyUSB0`가 보이는지 확인합니다.
3. RViz/Gazebo를 실행하기 전에 host에서 `xhost +local:docker`를 실행합니다.
4. 보존할 파일은 `/workspace` 아래에 저장합니다.
5. 컨테이너 내부의 `/root`, `/tmp` 등에 중요한 파일을 장기간 보관하지 않습니다.
