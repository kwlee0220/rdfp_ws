...
이를 위해 로봇 트윈에 필요한 상태 변수·연산을 정한다. 클라이언트는 `~/development/mdtpy/robot-twin-client` (사용 설명서 5.9 예제를 옮긴 별도 프로젝트) 처럼 **HTTP 만으로** 루프를 돌 수 있어야 한다 — 중간에 ROS 서비스를 직접 불러야 한다면 트윈의 전제가 깨진다.
