...
| [gripper/grasp_center_frame.md](gripper/grasp_center_frame.md) | **`grasp_center` 는 손끝이 아니라 파지 중심이다** — 정의가 `inner_finger` **링크 원점**의 중점이라 실제 손끝은 **약 40 mm 아래**(충돌 메시 44 mm)다. 모르고 파지 높이를 정했다가 손끝이 탁자를 파고든 사건(2026-09-09, `effort` 6.194 N·m)의 기록. 접촉 시작 높이 실측표, 파지 높이 산정법(**손끝 여유 44 mm 먼저, 그다음 패드 56.5 mm 가 물체를 덮는가**), 데카르트 목표를 `panda_link8` 로 역변환하는 절차(149 mm 차이), **작은 지령 데드밴드**(185 mm 이동 95.7 % vs 5.8 mm 보정 10 %), 정착을 기다려야 도달을 판정할 수 있다는 것(1.5 s 후 64.8 mm → 정착 후 −0.6 mm) | 현행 |
| [camera/compressed_image_pipeline_design.md](camera/compressed_image_pipeline_design.md) | **시뮬레이터가 압축(JPEG)으로 발행할 때 camera · image_viewer · image_recorder 를 어떻게 잇는가.** 가르는 기준(*픽셀이 필요한 소비자는 raw 를 구독하고, 바이트를 그대로 넘길 수 있는 소비자만 압축을 직접 받는다*), 뷰어가 직접 디코드하지 않는 이유(성능이 아니라 **소비자가 백엔드를 몰라야 한다**는 것 — `cv2.imdecode` 는 0.75 ms/frame 이고 GIL 도 놓는다), 프로파일의 `camera.source`(`device`/`compressed`/`native`)와 스위치 하나(`enable_camera`)로 합친 경위, 기본값이 raw 소비자에서 파생되는 이유(뷰어 인자에 묶여 있어 헤드리스 수집에서 레코더가 조용히 0 프레임을 담았다), `~/image_raw` 를 쓸 수 없는 이유(규약 §2.2 의 그리퍼 사고와 같은 실패), 대역폭 36배 표 | 현행 |
...
| 문서 | 내용 | 상태 |
|---|---|:-:|
| [session/session_control_guide.md](session/session_control_guide.md) | **⭐ 세션 제어 통합 가이드 (2026-09-06, 노드 + CLI + `SessionControlClient`)**. IDLE → IN_SESSION → IN_EPISODE 상태 머신, 서비스 6종(`stop_episode` 는 `StopEpisode` — `outcome`/`metadata` 를 실어 rosbag 에 남긴다) 과 `SessionCommand` 5필드, `/session_control/<name>` 서비스 경로와 루트 `/session` 토픽, `TRANSIENT_LOCAL` QoS. ⚠️ **depth 1 구독자는 2단계 전이의 `IN_SESSION` 을 놓친다(실측)** — `IDLE` 도 종료로 처리한다. 클라이언트: 동기/비동기 5 메서드, 서버 부재 시 즉시 실패, `wait_until_ready`, `timeout_sec<=0` 은 무한 대기. 옛 `session_control_client_guide.md` 를 흡수했다 | 현행 |
...
| [teleop/servo_vs_planned_motion.md](teleop/servo_vs_planned_motion.md) | **물체를 든 채 servo 로 올리면 옆으로 샌다 — 증상·원인·해결.** teleop 에서 물체를 집고 `q`(+z)를 누르면 위로 안 가고 옆으로 도는데(잡은 채 수평/수직 0.83, 놓으면 0.10, 트윈 `move_linear` 은 1.8 mm; status 는 내내 `NO_WARNING`), **원인은 되읽기(래칫)** — servo 에는 목표가 없어 매 주기 측정 관절값에 IK 증분을 더하므로 부하로 처진 만큼(100 g 에 joint5 주기당 7.7 mrad)이 명령에 적분된다; 명령을 IK 몫/되읽기 몫으로 가르면 +0.117/−0.337 로 IK 는 오히려 반대로 밀었다. 되읽는 지점이 둘(servo + 단일점 궤적마다 측정 상태에서 출발하는 JTC). **해결**: `servo_joint_source:=commanded`(`commanded_joint_state_node`) + JTC `open_loop_control: true`(`panda_isaac_ros2_controllers.yaml`) — 기본이며 0.83 → 0.01. 부작용으로 servo 가 직선 2.7배·회전 2.3배 빨라져 teleop `linear_step`/`angular_step` 기본을 0.25 로 되맞춤. §6 "servo 가 명령 없이도 흐른다"(09-06 관찰)는 09-07 재현 실패 — 옛 teleop 의 매 틱 0 발행이 원인일 것. §7 이력(09-06 이분법과 09-07 오전의 오판). **부록 A 재현·측정 절차** — 준비(teleop 금지, 원래 현상 재현용 launch 인자), 트윈으로 파지, `is_servo_push.py` 로 A/B/B2/C, status·joint5 분해 읽는 법, `is_servo_idle.py` 세 조건, 원자료 표(구성별 분해·속도 보정·무명령) | 현행 |
| [teleop/key_hold_programmers_guide.md](teleop/key_hold_programmers_guide.md) | **`HoldKeyTracker` — 키를 누른 동안만 명령이 나가게 하는 장치**. 터미널이 '뗐다'를 알려 주지 않아 TTL 로 추정해야 하는 이유와, 함께여야 동작하는 세 조각(버퍼 완전 비우기 · TTL · 만료 시 0 을 **한 번**). 사용 예제 5종(최소 루프 · rclpy 타이머 배선 · 입력원 바꿔 끼우기 · 정지 키와 `cancel()` · 다른 조작기), `HoldKeyTracker` / `HoldState` / `TerminalKeyReader` / `TerminalRawMode` API, 함정 5종. **§6 `ttl_sec` ↔ `xset r rate` 관계** — 두 인자가 각각 만드는 부등식(초기지연 < ttl, 반복간격 < ttl)과 반대 방향의 정지 지연, 실제 트래커에 자동반복을 먹여 잰 표(X 기본 660 ms 는 ttl 60 ms 의 11배라 시작 직후 한 번 끊긴다 — `xset r rate 30 30` 이면 0회), `ttl_sec` 이 아니라 자동반복을 고쳐야 하는 이유, 여유 2배 기준, evdev 전환 경로 | 현행 |
...
| 문서 | 내용 | 상태 |
|---|---|:-:|
| [scene/isaac_scene_reset.md](scene/isaac_scene_reset.md) | **Isaac 의 `/scene/reset` 구현** (2026-09-02). ROS 노드가 USD 스테이지를 직접 못 쓰므로 Isaac 이 여는 표준 `simulation_interfaces` 서비스(`/set_entity_state`)를 대신 부른다 — 시뮬레이터 쪽 상주 스크립트가 없다. ⚠️ **`result=1` 이 '적용됐다'를 뜻하지 않아**(rigid body 아닌 prim 은 성공 보고 후 아무 일도 안 한다) 판정을 `/get_entity_state` 읽기로 한다. **Isaac 은 만들지도 지우지도 않는다** — 빈 배열·모르는 이름·기하 불일치는 거부. 선행 조건 둘(`--enable isaacsim.ros2.sim_control`, **시스템 ROS 를 소싱하지 않는다** — Isaac 은 py3.12, Humble 은 3.10 이라 가려져 확장이 죽는다)과 실기 검증 기록 | 현행 |
...
| [simulation/multi_simulator_backend_design.md](simulation/multi_simulator_backend_design.md) | mock / Gazebo / Isaac Sim 을 교체 가능한 백엔드로 꽂는 구조 설계. **§0 「실제로 만든 것」(2026-09-08)** — §4 의 launch 재배치는 채택하지 않고, 대신 백엔드별 값(`config/backends/*.yaml`)과 코드(`robot_control/backends/`)를 모으는 층을 두었다. 왜 그쪽이 아니었는지와 적용 범위·검증 여부 표. 문서 자체가 "코드 미반영" 명시 | 일부 실현 |
| [src/robot_control/config/backends/README.md](../src/robot_control/config/backends/README.md) | **백엔드 프로파일 — 백엔드 하나 = 파일 하나**. 블록 스키마(`arm_command`/`ros2_control`/`simulator`/`servo`/`camera`/`scene`/`use_sim_time`)와 `null` 의 뜻('이 백엔드에는 없다'), **`arm_command.topic` 과 `ros2_control.joint_commands_topic` 이 다른 것**인 이유(옛 bridge 시절 섞어 두어 컨트롤러를 우회해 싸웠다), 경로가 패키지 share 기준이라 워크스페이스 루트의 녹화 목록은 못 담는다는 것, `factory:` 로 백엔드 고유 코드를 다는 법, **파일을 지웠을 때 colcon 이 설치본을 안 치우는 함정** | 현행 |
| [simulation/functionbay_backend_design.md](simulation/functionbay_backend_design.md) | **펑션베이 백엔드 (토픽 연동형) 설계**. ros2_control 없이 MoveIt 스택을 올리는 구성 — `joint_state_fusion`(이름 없는 배열에 이름 부여) · `readiness_gate`(spawner 대체, 종료코드 게이팅). 보간 없는 시뮬레이터라 `publish_rate=50` 이 필수인 근거(10 Hz vs 50 Hz 실측), 그리퍼가 Robotiq 2F-85 라 액션이 아니라 관절 목표각 토픽으로 움직이는 경위(`robotiq_2f_gripper_node` — 배선·측정 완료), 수집 계층 조합 실측  **2026-09-04 정리** — `open_work` §7 의 정정 대기 항목 **10건을 전부 흡수**했다. §2 표(팔 주기 XML 설정값 · `velocity`/`effort` 채워짐 · endeffector 가 `PoseStamped` 로 변경 · 카메라 실시간 렌더링과 `camera_info` 부재), **§4.2 판별 조건을 코드와 일치**시킴(`publishers or subscribers` — 토픽 이름 존재가 아니다. 우리 퍼블리셔가 판별을 뒤집는다), §4.4 방향 의존 최신값(비대칭 20.0 → **9.87 mm** 정체) + **측정 프레임 주의**(tip `panda_link8` 기준 — `/ee_pose` 로 데카르트 목표를 만들면 149.2 mm 어긋난다), §6.1 에 **파지가 정지 후 6.5초 뒤 붕괴**한다는 B-16 주, §9 미결 항목(그리퍼 description 교체 · 손끝 접촉 정의) — **scene 은 2026-09-08 완료**(물체 TF → `functionbay_scene_state_node`, 단 `/scene/reset` 은 미제공) | 현행 |
| [simulation/functionbay_open_work.md](simulation/functionbay_open_work.md) | **펑션베이 백엔드 인수인계 — 남은 작업**. 3회 실측(08-18 / 08-28 / **08-31 — 솔버가 `펑션베이` → `서울대` 로 교체됨**). §0.1 에 **두 솔버 비교표**, §0.2 에 **수집용 솔버 권장(서울대) 과 근거** — 정확도는 10배 개선(처짐 0.0983 → 0.0097 rad), 응답은 1.8배 악화(τ 133 → 242 ms), 보고 주기 50 → 10 → 30 Hz(카메라는 9.6 Hz 로 무관), `/output/endeffector` 복구. servo 단순 브리지는 **발산이 사라진 대신 추종 실패**(명령 방향과 무관하게 EE 가 아래로 흐름). 간헐적 고착은 후보가 **Unity 수신 경로 하나로 축소**(엔드포인트 재시작·EE 명령 모두 무효 확인). 그 밖에 자기충돌, 제어기 강성 모델(오차 = 중력토크/K_p), 재현 절차, 진단 스크립트 6종  **2026-09-04 정리 — §0 머리에 "닫힌 항목" 표를 넣었다**: 시계 오프셋 2.22 s(폐기) · 카메라 stamp/5 Hz · `velocity`·`effort` · servo 무추종 · 설계서 `name` 서술 · `/output/endeffector`(우리가 쓰지 않기로 확정해 무효) · 간헐적 고착(재현 없음) · 그리퍼 브리지 · A-5 발송 — 총 10건이 닫혔고 본문은 취소선 처리. **열린 것은 6건**(중력 처짐 방침 · 링크 메시 · 설계서 정정 2건 · 자기충돌 가드 · 수집 계층 재확인 · scene). **최신 실측값의 정본은 checklist 의 A·B 표다** | 현행 |
| [simulation/functionbay_vendor_requests.md](simulation/functionbay_vendor_requests.md) | **펑션베이 벤더 요청 항목**. `functionbay_open_work.md` 실측에서 "시뮬레이터 쪽에서만 고칠 수 있는" 것만 뽑아 요청 단위로 재구성 — 각 항목이 *무엇을 · 왜 우리가 못 고치나 · 수용 기준* 세 가지를 갖춘다. 필수 5건(**중력 보상**·제어기 사양 공개·명령 무반응 고착 문의·**A-5 34초 주기 2초 정지**)과 후속 항목들. **§0.2 (2026-09-03) 에서 A-3 카메라 `stamp`·B-7 NTP·B-10 카메라 주기·`velocity`/`effort` 4건이 닫히고 A-5 가 열렸다** — 발송 전 §0.1·§0.2 를 먼저 본다. 검증 스크립트 절차와 **요청하지 않는 것**(이미 정상인 8항목 — 특히 `/clock` 은 불필요) 포함  **2026-09-04 정리** — §0.3 신설(전수 측정 결과 대장). **살아 있는 요청은 A-1·A-2·A-5·B-2·B-8·B-9·B-11·B-12 여덟 건**이고 나머지는 취소선 처리. A-4(재현 없음)·B-1(τ 199.9 ms 개선)은 **강등**, B-4 는 **철회**(`/output/endeffector` 미사용 확정), B-11(손끝 접촉)·B-12(물체 pose TF)·`effort` 의미 문의가 **신규**. **A-5 는 2026-09-03 발송·회신 대기**, 나머지 3건 미발송. §0.1 은 이력으로 격하 (그 표의 B-1 "악화"·A-4 "재현" 두 칸은 뒤집혔다) | 현행 |
| [simulation/functionbay_checklist.md](simulation/functionbay_checklist.md) | **⭐ 펑션베이 인수 체크리스트**. 시뮬레이터가 갱신될 때마다 *무엇이 달라졌는가*를 가려내는 절차서 — 판정을 개선/회귀/**변경**(통보 없는 사양 변경, 가장 위험) 셋으로 나눈다. A(수동 관측 18항목, 팔 안 움직임) · B(능동 **16항목**, 승인 필요 — 2026-09-04 전수 실행) · C(스택 통합) · D(벤더 요청 상태) · E(다음에 볼 것) · F(기준선 이력). **§0 의 측정 함정 9가지**가 핵심 — `ros2` 데몬 실명(재시작으로도 안 낫는다), 톱니를 지터로 오독, 진행률 끝점 추정 금지, 발행 주기는 XML 설정값. **A-17 = 34초 주기 2초 정지** 상세 포함 — **정지 길이는 세션마다 2.04 → 0.9 s 로 변한다**(벤더 미반영 상태에서). **2026-09-04 A·B 전수 갱신**: A 표 18행 재측정, B 16항목 완주, `effort = 200 × 잔차` 해석과 함정 5 정정. **2026-09-08 새 빌드**: 카메라가 `CompressedImage` 로 바뀐 §A-6 상세(결정 ② 소비자를 압축으로 — 레코더는 `input_format='mjpeg'`, 뷰어는 `image_transport/republish` 로 해결), `/clock` 은 발행되나 시간 기준이 섞여 **사용 불가**(§A-15), 관절 주기 50 Hz 재설정 | 현행 |
| [robot_twin/peg_in_hole_via_mcp_plan.md](robot_twin/peg_in_hole_via_mcp_plan.md) | **⭐ peg-in-hole 을 트윈·MCP 로 — 계획서 겸 작업 기록. 7단계 전부 완료 (2026-09-11).** *"peg 을 다른 hole 로 옮겨서 꽂아줘"* 가 **MCP 도구 호출로 된다** — 왕복 5회 연속·이탈 0건, 삽입 오차 0.05~0.40 mm. §0 진행 현황표에 단계별 실측 근거가 있다. **값진 것은 실패 기록이다** — 트레이 삽입이 네 번 연속 같은 자리에서 막혔고, 정렬·모따기·파지력·팔의 횡방향 흐름을 차례로 의심했다가 **전부 아니었다**: 원인은 **팔(1 Hz)과 그리퍼(계단) 양쪽 모두 지령이 끊겨 나가던 것**이었고, 둘 다 시험으로도 `/health` 로도 안 잡혀 **사람이 움직임을 보고 알려 줘서** 찾았다. 그 밖에 파지 판정이 지터 대역에 걸려 동전던지기였던 것, 시뮬레이터를 프로세스째 재시작하면 ROS 스택도 다시 띄워야 하는 것(TF_OLD_DATA), 기운 peg 은 바로 세울 수 없다는 것. **코드로 굳힐 것과 프롬프트로 알릴 것의 경계**가 설계의 핵심 판단이다 | 현행 |
| [simulation/functionbay_gravity_compensation.md](simulation/functionbay_gravity_compensation.md) | **⭐ 펑션베이 중력 보상**. 팔이 지령보다 처지는 문제를 우리 쪽에서 상쇄한 기록 — 증상(수직 이동 시작 시 손끝이 안쪽으로 7 mm 말렸다 풀림, peg 이 씬을 이탈 4/4), 원인(`joint2`·`joint4` 가 0.2~0.4초에 뒤처지고 **복구되지 않는다**; 부하 성분이지 속도 성분이 아니다), 해결(`Kp` 를 씬 XML 이 알려 주므로 `−τ_g(q)/Kp` 를 모든 명령점에 더한다 — `ready` 예측이 실측과 **일곱 관절 0.03 mrad** 일치, 8지점 도달 오차 **12.37 → 0.00 mm**, 말림 **−7.14 → −0.01 mm**), 한계(부하·속도 성분 · 검증 범위), 그리고 **벤더가 A-1 을 반영했을 때 되돌리는 절차**(판별 → yaml 한 블록 삭제 → 부분 반영이면 `scale`). 이것이 지점별 처짐 캘리브레이션을 대체했다 — **함께 켜면 이중 보정** | 현행 |
| [simulation/functionbay_vendor_mail_20260903_stall.md](simulation/functionbay_vendor_mail_20260903_stall.md) | **벤더 발송 문안 — 시뮬레이터 주기적 정지**(2026-09-03 작성, **발송 완료 · 회신 대기**). 34.3초 주기 2.0초 정지 현상의 측정값(두 발행 주기 설정 비교), 시뮬레이터 측 현상이라는 근거 4가지, 상시 6~7% 과속이 캐치업이 아니라는 근거, 실시간성 영향 3가지, 요청·수용 기준. **데이터 결손은 아니다** — sim 시각 기준 stamp 는 연속 | 현행 |
| [simulation/isaac_bringup_runbook.md](simulation/isaac_bringup_runbook.md) | **⭐ Isaac + 로봇 스택 기동 절차.** 세 모드(`--gui` 창+자동 · `--headless` 검사/CI · 인자 없음은 전체 편집기일 때만)와 터미널별 명령, **Script Editor 스크립트 일곱 개의 순서·역할·빠뜨리면 생기는 증상** 표, 수용 기준 검사 목록, `kill_stack.sh` 정리. 자주 물리는 함정 넷 — **시뮬레이터를 재시작하면 스택도 다시 띄운다**(TF 버퍼가 옛 타임스탬프를 들고 있어 `/scene/objects` 가 직전 실행의 자리를 말한다), `ros2 launch` 를 죽여도 자식 노드가 남는다, 스테이지를 저장하지 않는 이유(USD 와 `isaac_scene.json` 이 갈라진다), 로봇이 원점에 있어야 하는 이유 | 현행 |
| [simulation/isaac_backend_skeleton.md](simulation/isaac_backend_skeleton.md) | **Isaac Sim 백엔드 — 설계·구현·실측 (Phase 0~11 완료, 2026-09-05)**. 연동 방식은 `topic_based_ros2_control` **하나**다(D1″ — 토픽 브리지 판은 삭제). 골격 → 팔(정착 0.3 mrad) → 그리퍼 → scene → 카메라 → **물리 파지**(+0.0999 m) → **수집 전 과정**(세션→scene reset→에피소드→rosbag→mp4→DB) → 트윈 REST. plugin 의 대가도 실측 — 배속 −5%, load 3배, dead time +100 ms. §7 함정 사전 20건, §8 열린 결정 5건 | 현행 |
| [simulation/isaac_windows_setup.md](simulation/isaac_windows_setup.md) | **새 Windows 11 머신 구축 절차 (Windows Isaac + WSL2 스택 — §1 부록)**. Isaac Sim 만 설치된 상태에서 처음부터 돌리기까지 8단계 — WSL2/Ubuntu-22.04, `.wslconfig`(메모리 캡·mirrored networking), ROS 2 Humble 의존 목록, 워크스페이스 빌드, **Isaac 쪽은 파일 2개 복사**(`run_isaac_humble.bat` + `fastdds_wsl_bridge.xml`), 환경변수 4개, **sim_side 스크립트 6개의 실행 순서와 각각이 없으면 생기는 증상**, 검증 순서. 자주 막히는 곳 7가지를 증상 → 원인 → 확인법 표로 정리 — **셋이 모두 '에러 없이 토픽이 안 보이는' 증상이라 눈으로 구별되지 않는다**. 근거·설계는 isaac_backend_skeleton.md 에 있고 이 문서는 재현 절차만 담는다 | 현행 |
...
| [robot_twin/mcp_server_design.md](robot_twin/mcp_server_design.md) | **로봇 트윈 MCP 서버 설계서**. LLM 에이전트가 대화만으로 로봇 상태를 읽고 연산을 조합하게 하는 경로(`~/development/mdtpy/robot-twin-client` 의 `mcp_server.py`). **트윈이 자기 능력을 알리기 위한 변경**(`config.py`/`api.py` 에 `description` 필드 + 카탈로그 노출, 설정 YAML 19건 작성)과 그 필요성(`extra='forbid'` 라 YAML 만으로는 기동 실패), 도구 18개의 생성 규칙(카탈로그 자동 생성 5 + 검증 래퍼 교체 2 + 추가 11, 감춤 4), 결정 13건(상태 변수를 resource 가 아닌 tool 로 노출한 이유, 여는 연산만 감춘 비대칭, 작업 상태를 서버가 소유하고 metadata 자동 병합, SDK 2.0 에서 low-level API 를 쓴 경위), 이슈(인증 없는 경로 위임·카탈로그 캐시·동시 호출 무보호·타임아웃 2층) | 현행 |
...
| **카메라 노드 셋 중 무엇을 쓸지 고른다** | [camera/README.md](camera/README.md) |
...
| 세션에 맞춰 카메라를 켜고 끈다 | [camera/rdfp_camera_node_guide.md](camera/rdfp_camera_node_guide.md) |
...
| 새 시뮬레이터 빌드를 받아 무엇이 바뀌었는지 확인한다 | [simulation/functionbay_checklist.md](simulation/functionbay_checklist.md) |
| 주기적 정지 현상을 벤더에 보낸다 | [simulation/functionbay_vendor_mail_20260903_stall.md](simulation/functionbay_vendor_mail_20260903_stall.md) |
