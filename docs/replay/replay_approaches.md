...
- **반응형이 아니다.** 물체가 녹화 때와 다른 곳에 있어도 같은 관절 궤적을 그대로 따라간다 — 허공을 집는다. 씬이 고정된 재생에만 맞는다.
...
**적합한 경우**: 같은 로봇·같은 씬에서 동작을 있는 그대로 되살릴 때(데모 재생, teaching 동작 재현). 팔꿈치 형상까지 맞아야 한다면 사실상 이 방법뿐이다.
...
- 반대로 **C 는 씬이 바뀌면 대응하지 못한다.** 물체가 다른 곳에 있어도 같은 관절 궤적을 따라가므로, 물체 위치가 매번 달라지는 수집 데이터의 재생에는 맞지 않는다.
