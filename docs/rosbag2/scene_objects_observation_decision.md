...
| 제약 | 현재 | 해소 시점 |
|---|---|---|
| **scene 카메라 부재** | [image_pipeline.yaml](../../src/robot_control/config/image_pipeline.yaml) 의 `camera.id` 가 mp4 파일이다 — 실 카메라가 연결되지 않아 쓰는 임시 소스이며 scene 과 무관한 영상이다 | 실 카메라 연결 시 |
...
- 실 카메라가 연결되어 scene 을 담기 시작할 때 → §5 의 첫 제약이 사라지므로 A 를 실측한다
