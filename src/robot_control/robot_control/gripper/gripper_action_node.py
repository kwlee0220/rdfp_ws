...
계약 전문은 `docs/gripper/GripperNode_Design.md` §3 에 있다. 이 노드가 지키는 것:
...
class GripperActionNode(Node):
    ...
    def _at_goal(self) -> bool:
        ...
        **폭으로 재는 것은 계약이 아니라 이 구현의 선택이다.** 관측이 손가락 관절
        하나뿐이라 그렇게 한다. 계약이 요구하는 것은 "목표 자세 도달"이고, 지령과
        보고가 같은 관절 공간인 백엔드는 관절 잔차로 재는 편이 정확하다 — 그런
        구현은 ``width`` 가 NaN 이어도 ``at_goal`` 을 판정할 수 있다.
