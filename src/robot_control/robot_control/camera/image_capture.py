...
def main(args=None):
    ...
    SIGINT / SIGTERM 은 각각 ``KeyboardInterrupt`` / ``ExternalShutdownException``
    으로 올라온다. 후자를 잡지 않으면 빈 메시지의 'Node exception' 만 남고,
    이미 내려간 컨텍스트에 ``rclpy.shutdown()`` 을 다시 부르면서 ``RCLError``
    traceback 으로 끝난다 — 그래서 ``try_shutdown()`` 을 쓴다.
    """
