...
class ServoClient:
    ...
    def wait_for_services_ready(self, timeout_sec: float = 10.0) -> bool:
        ...
        self.logger.warning(
            f"[servo_node] ⏰ Timeout waiting for servo services after {timeout_sec}s")
        return False

    def call_servo_service(self, client: Client, service_name: str,
                           timeout_sec: float = 5.0) -> Tuple[bool, str]:
    ...
    def check_status(self, wait_for_status: bool = True,
                     timeout_sec: float = 3.0) -> Tuple[ServoStatus, str]:
