...
from rdfp.ros2_utils import await_future_spin, await_future_spin_nospin
...
class TrajectoryStreamer:
    ...
    def __init__(self, node: Node, *, command_topic: str = DEFAULT_COMMAND_TOPIC,
        ...
        self._closed = False
        self._publisher = node.create_publisher(Float64MultiArray, self._command_topic, qos_depth)
        ...
        try:
            for target_elapsed, data in samples:
                if not _sleep_until(started_at + target_elapsed, events):
                    ...
                    return published
                self._publisher.publish(Float64MultiArray(data=data))
                published += 1
        finally:
