...
본 모듈이 rclpy 와 :mod:`rdfp.moveit` 을 잇는 유일한 지점이다. API 계층은 이
...
from rdfp.twin.backends import PreconditionFailed
from rdfp.twin.config import OperationConfig, TwinConfig
from rdfp.twin.errors import (
    ErrorCode, cancel_unsupported, invalid_input, precondition_failed
)
from rdfp.twin.session import Phase, Session, SessionStore, Status
from rdfp.twin.variables import StaticSourceUnavailable, VariableCache
...
class RobotTwinRuntime:
    ...
    def _start_command_publishers(self) -> None:
        ...
        """
        from rdfp.twin.variables import import_message_type
    ...
    def session_control(self) -> Any:
        ...
        **`wait_timeout_sec=0` 으로 만든다.** 생성자의 기본 동작은 6개 서비스가
        ...
        if self._session_control is None:
            ...
            self._session_control = SessionControlClient(self._node, wait_timeout_sec=0.0)
    ...
    def start_move_group_async(self) -> None:
        ...
        def _build() -> None:
            ...
            try:
                ...
                from rdfp.moveit.move_group_factory import create_move_group_client
    ...
    def _fetch_static(self, method: str, timeout: float) -> Any:
        ...
        :class:`~rdfp.twin.variables.VariableCache` 에 주입되는 resolver 다. 캐시가
    ...
    def _check_preconditions(self, op: OperationConfig) -> None:
        ...
        **연산이 실제로 쓰는 백엔드만 본다.** 그리퍼 연산은 액션을 직접 호출하므로
        ...
        """
        from rdfp.twin.backends import requires_move_group
    ...
    def _validate_inputs(self, op: OperationConfig, inputs: dict[str, Any]) -> None:
        ...
        from rdfp.twin.backends import validate_inputs as validate_domain
        try:
    ...
    def _dispatch(self, op: OperationConfig, session: Session, *,
        ...
        """
        from rdfp.twin.backends import dispatch
        with self._mg_lock:
            client = self._move_group
