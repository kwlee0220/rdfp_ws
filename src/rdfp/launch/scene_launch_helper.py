"""씬 상태 노드(백엔드별 scene_state_node) 및 파라미터 설정.

씬 노드는 백엔드와 **1:1 로 묶인다** — mock 스택은 `mock_scene_state_node`,
...
씬 노드는 `move_group` 이 제공하는 `/monitored_planning_scene` 과
...
기반 선언). 씬 노드가 발행하는 물체 pose 의 기준 프레임은 EE pose 의 기준
...
def declare_scene_arguments() -> list[DeclareLaunchArgument]:
    """씬 상태 노드 관련 launch argument 들을 선언한다.
    ...
    return [
        DeclareLaunchArgument(
            ...
            description=(
                ...
                "(/scene/objects, /scene/commands, /scene/command_results)"
            ),
        ),
        DeclareLaunchArgument(
            "scene_publish_rate",
...
def create_mock_scene_node() -> Node:
    """mock 백엔드(MoveIt planning scene) 의 씬 상태 노드를 생성한다.
